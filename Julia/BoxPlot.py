...
data0.head()
No_Up = data0["No_Up"]
Single_Up = data0["Single_Up"]
Double_Up = data0["Double_Up"]

#### For not change first #######
...
fvalue, pvalue = stats.f_oneway(data0['No_Up'], data0['Single_Up'], data0['Double_Up'])
print("One way anova")
...
d_melt = pd.melt(data0.reset_index(), id_vars=['index'], value_vars=['No_Up', 'Single_Up', 'Double_Up'])
...
for i in range(len(sigma_1_list)):
    ...
    xmin, xmax = ax.get_xlim()
    plt.plot(np.linspace(xmin, xmax, 100), np.percentile(No_Up, 95)*np.ones(100), "--", color = "black", linewidth = 2)  
    plt.text(2.75, 1.1*np.percentile(No_Up, 95), "95th percentile (No-I)", fontsize = 15)
    plt.ylabel("MI scores", fontsize = 20)
